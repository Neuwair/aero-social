...
import { getSimulationState } from "../utils/database.js";
...
export const handler = async (event, context) => {
  ...
  try {
    ...
    try {
      console.error(
        "api-getSimulationState error:",
        e && e.stack ? e.stack : e
      );
    } catch (err) {
      console.error("api-getSimulationState error (failed to print stack):", e);
    }
    const message =
      process.env.NODE_ENV === "production"
        ? "Failed to get simulation state"
        : (e && e.message) || "Failed to get simulation state";

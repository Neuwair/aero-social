...
import { listPendingRequests } from "../utils/database.js";
...
export const handler = async (event, context) => {
  ...
  try {
    const rows = await listPendingRequests(user.uid);
    return respond(200, { success: true, pending: rows });
  } catch (e) {
    console.error("api-listPendingRequests error:", e);
    return respond(500, {
      success: false,

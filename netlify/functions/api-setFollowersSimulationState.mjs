...
export const handler = async (event, context) => {
  ...
  try {
    await setFollowersSimulationState({
      userUid: user.uid,
      activeElapsedMs,
      lastStartedAt,
      stopAt,
    ...
    try {
      console.error(
        "api-setFollowersSimulationState error:",
        e && e.stack ? e.stack : e
      );
    } catch (err) {
      console.error(
        "api-setFollowersSimulationState error (failed to print stack):",
        e
      );
    }
    const message =
      process.env.NODE_ENV === "production"
        ? "Failed to set followers simulation state"
        : (e && e.message) || "Failed to set followers simulation state";
    return respond(500, {
      success: false,
      message,
    });
